...
# FKAConv imports
from fkaconv.networks.kpconv import KPConvCls as Network
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", "-c", type=str, help="Path to config file in savedir")
    parser.add_argument("--iter", "-i", type=int, help="Number of iter per shape (if not specified, use from config.yaml)", default=-1)
    parser.add_argument("--batchsize", "-b", type=int, help="Batchsize (if not specified, use from config.yaml)", default=-1)
    ...
    if args.iter > 0:
        config["test"]["num_iter_per_shape"] = args.iter

    if args.batchsize > 0:
        config["test"]["batchsize"] = args.batchsize
