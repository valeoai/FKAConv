...
from lightconvpoint.datasets.npm3d import NPM3D_Pillar_Test as Dataset
...
def main(_config):
    ...
    for file_id in range(ds.size()):
        ...
        if _config['test']['savepts']:
            ...
            save_fname = os.path.join(savedir_root, f"results_{step}_pts", filename+"_decimated.txt")
            np.savetxt(save_fname, xyzrgb[::10], fmt=['%.4f','%.4f','%.4f','%d'])
